...
public class Barrel extends NonHopperableBlock implements DoubleHologramOwner {
    ...
    private final ItemStack HOLOGRAM_OFF_ITEM = new CustomItemStack(Material.QUARTZ_SLAB, "&3Hologram Display &c(Off)");
    private final ItemStack HOLOGRAM_ON_ITEM = new CustomItemStack(Material.QUARTZ_SLAB, "&3Hologram Display &a(On)");
    private final ItemStack TRASH_ON_ITEM = new CustomItemStack(SlimefunItems.TRASH_CAN, "&3Void Excess Input &a(On)",
        "&7When enabled, items that cannot fit will be voided");
    private final ItemStack TRASH_OFF_ITEM = new CustomItemStack(SlimefunItems.TRASH_CAN, "&3Void Excess Input &c(Off)",
        "&7When enabled, items that cannot fit will be voided"
    );
            ...
            public void onPlayerBreak(@Nonnull BlockBreakEvent e, @Nonnull ItemStack item, @Nonnull List<ItemStack> drops) {
                ...
                if (inv != null) {
                    ...
                    if (breakOnlyWhenEmpty.getValue() && stored != 0) {
                        Utils.send(p, "&cPlease remove the items inside before breaking it!");
                        e.setCancelled(true);
                        return;
                    }
                    ...
                    if (itemCount > 5) {
                        Utils.send(p, "&cYou should remove the items inside before breaking it!");
                        e.setCancelled(true);
                        return;
                    }
                    ...
                    if (stored > 0) {
                        ...
                        if (stored > OVERFLOW_AMOUNT) {
                            ...
                            Utils.send(p, "&eThe Fluffy Barrel contains more than " + OVERFLOW_AMOUNT + " items! " +
    ...
    protected void buildMenu(BlockMenu menu, Block b) {
        ...
        menu.replaceExistingItem(INSERT_ALL_SLOT,
                new CustomItemStack(Material.LIME_STAINED_GLASS_PANE, "&bQuick Deposit",
                        "&7> Click to deposit all available items", "&7from your inventory into the Fluffy Barrel"));
        ...
        menu.replaceExistingItem(EXTRACT_SLOT,
                new CustomItemStack(Material.RED_STAINED_GLASS_PANE, "&6Quick Extract",
                        "&7> Left-click to fill your inventory",
                        "&7> Right-click to extract one item"
                ));
    ...
    public void extract(Player p, BlockMenu menu, Block b, ClickAction action) {
        ...
        if (action.isRightClicked()) {
            ...
            Utils.send(p, "&cThe Fluffy Barrel is empty!");
            return;
        }
    ...
    public enum BarrelType {
        ...
        SMALL(17280000, "&eTier 1 Fluffy Barrel", Material.BEEHIVE, SlimefunItems.REINFORCED_PLATE, new ItemStack(Material.OAK_LOG)),
        MEDIUM(34560000, "&6Tier 2 Fluffy Barrel", Material.BARREL, SlimefunItems.REINFORCED_PLATE, new ItemStack(Material.SMOOTH_STONE)),
        BIG(69120000, "&bTier 3 Fluffy Barrel", Material.SMOKER, SlimefunItems.REINFORCED_PLATE, new ItemStack(Material.BRICKS)),
        LARGE(138240000, "&aTier 4 Fluffy Barrel", Material.LODESTONE, SlimefunItems.REINFORCED_PLATE, new ItemStack(Material.IRON_BLOCK)),
        MASSIVE(276480000, "&5Tier 5 Fluffy Barrel", Material.CRYING_OBSIDIAN, SlimefunItems.REINFORCED_PLATE, new ItemStack(Material.OBSIDIAN)),
        BOTTOMLESS(1728000000, "&cTier 6 Fluffy Barrel", Material.RESPAWN_ANCHOR, SlimefunItems.BLISTERING_INGOT_3, SlimefunItems.REINFORCED_PLATE);
