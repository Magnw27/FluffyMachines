...
public class AutoCraftingTable extends SlimefunItem implements EnergyNetComponent {
    ...
    public AutoCraftingTable(ItemGroup category, SlimefunItemStack item, RecipeType recipeType, ItemStack[] recipe) {
        ...
        new BlockMenuPreset(getId(), "&6Auto Crafting Table (Vanilla)") {
            ...
            public void newInstance(@Nonnull BlockMenu menu, @Nonnull Block b) {
                ...
                if (keyItem == null) {
                    menu.replaceExistingItem(KEY_SLOT, new CustomItemStack(Material.BARRIER, "&cNo Recipe", "&cHold item and Shift+Right-Click",
                            "&cto set the item to craft"
                    ));
                } else {
                    ItemMeta keyMeta = keyItem.getItemMeta();
                    List<String> lore = keyMeta.getLore();
                    if (lore == null || !ChatColor.stripColor(lore.get(0)).equals("Hold item and Shift+Right-Click")) { // Check if item has been replaced
        ...
        for (int i = 0; i < 9; i++) {
            ...
            if (slotItem.getAmount() == 1) {
                if (menu.hasViewer()) {
                    menu.replaceExistingItem(statusSlot,
                            new CustomItemStack(new ItemStack(Material.RED_STAINED_GLASS_PANE),
                                    "&c&lYou need to place more items", "&c&lto craft"));
                }
                return;
            }
    ...
    public static ItemStack createKeyItem(Material material) {
        return new CustomItemStack(material, "", "&cHold item and Shift+Right-Click",
            "&cto set the item to craft"
        );
    }
}
